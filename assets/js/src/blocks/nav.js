$(document).ready(function() {
  ...
  function _getDefaultTabName() {
    var defaultTabHash = $(sections).filter(function(i, section) {
  ...
  function _isCorrectTab(tabName) {
    var existingTab = $(sections).filter(function(i, section) {
      return section.id == $.trim(tabName);
    })[0];
  ...
  function _activateCurrentTab() {
    ...
    $("nav#menu a").filter(function(i, aTab) {
      return $(aTab).attr('href') == window.location.hash
    }).addClass('active');
  }
